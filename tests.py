#Description:
#Login: Logs into the site using the username and password.
#Product List: Verifies that the product list page is displayed.
#Add to Cart: Adds an item (Backpack) to the shopping cart.
#Checkout: Fills out the checkout form and verifies the overview.
#Logout: Attempts to log out and verifies if the user is logged out.
#Failure Test: Deliberately searches for a non-existent element to test failure handling.
#Excel Report: Generates an Excel file with a report of the test steps and their status.
